...
describe('StartPage Component', () => {
  ...
  it('renders the main heading and introductory text', () => {
    ...
    // Check paragraphs
    expect(
      screen.getByText(/Evidence by Commission \(EBC\) is a legal procedure/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/To support the EBC process, Justiciary teams can now use/i)
    ).toBeInTheDocument();
  });
  ...
  it('renders the start button', () => {
    ...
    // Check button exists
    expect(
      screen.getByRole('button', { name: /Start the transcription upload service now/i })
    ).toBeInTheDocument();
  });
});
