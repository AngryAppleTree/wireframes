...
describe('UploadPage Component', () => {
  it('should not have any accessibility violations', async () => {
    const { container } = render(
      <UploadPage onUploadComplete={() => {}} onBack={() => {}} onCancel={() => {}} />
    );
  ...
  it('renders title and dynamic instructions derived from config', () => {
    ...
    expect(
      screen.getByText(/Drag & drop a file or click "Choose file" to browse your device/i)
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText((_content, element) => {
        const hasText = (node: Element) =>
          node.textContent === `Supported formats: ${allowedExtensionsText}`;
  ...
  it('displays an error message when submitting without a file', async () => {
    ...
    render(
      <UploadPage onUploadComplete={handleUploadComplete} onBack={() => {}} onCancel={() => {}} />
    );

    await userEvent.click(screen.getByRole('button', { name: /Upload selected file/i }));

    expect(
      screen.getByText(/Error: No file selected. Please select a file before continuing./i)
    ).toBeInTheDocument();
